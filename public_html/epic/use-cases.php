<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8"/>
		<title> Use cases page</title>
	</head>
	<body>
		<h1>Use cases </h1>
				<h2>Bill Pruitt</h2>
					<ul>
						<li>Bill accesses the site on his iPhone6.</li>
						<li>Bill favorites his favorite team and/or player</li>
						<li>Bill has already configured his profile to bring up the content for the teams he follows, i.e. Viking, Twins.</li>
						<li>Bill sees content about his team that interests him and he clicks on the link.</li>
						<li>The site brings up the content about the team/player</li>
					</ul>
				<h2>Jude Chavez</h2>
					<ul>
						<li>Jude accesses the site on his work computer that runs IE.</li>
						<li>Jude does not have a profile to bring up the content for the teams he follows, i.e. Viking, Twins. but he can see and search content anyway.</li>
						<li>Jude sees content about his query and he clicks on the link.</li>
						<li>The site brings up the content about the team.</li>
	</body>

</html>
